...
Category pages will discover entries from front matter and use an explicit `order`; each entry will provide a stable `slug` that is independent of its source filename. The first build will copy shared assets unchanged, keep theme-candidate pages outside the main build, and place only publishable HTML and assets in `dist/`.
