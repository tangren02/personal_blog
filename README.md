...
首页、知识总览和三个分类页，以及 `projects/hstring/`，现在都由生成器写入 `dist/`。站点级导航事实保存在 `content/site.yaml`，具体学习条目仍使用带 front matter 的 Markdown。分类页从条目元数据发现入口并处理空分类。HString 页面中的内存图和执行轨迹暂时以受控 HTML 保存在 Markdown 中。
